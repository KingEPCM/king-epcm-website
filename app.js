...
(function () {
  ...
  function renderTasks(el, items) {
    if (!el) return;
    if (!items.length) {
      el.innerHTML = '<div class="note" style="margin:0">🎉 Nothing overdue or due in the next two weeks.</div>';
      return;
    }
    var today = new Date(); today.setHours(0, 0, 0, 0);
    var overdue = [], dueToday = [], later = [];
    items.forEach(function (t) {
      if (!t.due) { later.push(t); return; }
    ...
    var groups = [
      { label: "Overdue", items: overdue, cls: "late", open: true, empty: "Nothing overdue." },
      { label: "Due today", items: dueToday, cls: "warn", open: true, empty: "Nothing due today." },
      { label: "Due later", items: later, cls: "ok", open: false, empty: "Nothing else coming up." }
    ];
    el.innerHTML = groups.map(function (g) {
      var body = g.items.length
        ? g.items.map(taskItemHtml).join("")
        : '<div class="note" style="margin:0">' + g.empty + '</div>';
      return '<details class="proc-group task-group"' + (g.open ? " open" : "") + '>' +
        ...
        '<div class="proc-group-body">' + body + '</div></details>';
    }).join("");
  }
  ...
  function initTeamwork() {
    ...
    var projLimit = projEl ? parseInt(projEl.getAttribute("data-limit") || "0", 10) : 0;
    var taskLimit = taskEl ? parseInt(taskEl.getAttribute("data-limit") || "0", 10) : 0;

    function render(data) {
      var projects = data.projects || (Array.isArray(data) ? data : []);
      var tasks = data.tasks || [];
      applyProjects(projEl, projects, projLimit);
      renderTasks(taskEl, tasks);
    }
    ...
    fetch("/api/teamwork-projects")
      ...
      .catch(function () {
        ...
        renderTasks(taskEl, SAMPLE_TASKS);
      });
  }
  function applyProjects(el, items, limit) {
    if (!el) return;
    _allProjects = items;
    renderProjects(el, limit ? items.slice(0, limit) : items);
  ...
  function initProcedures() {
    ...
    if (!el) return;
    var limit = parseInt(el.getAttribute("data-limit") || "0", 10);
    fetch("procedures.json").then(function (r) { return r.json(); }).then(function (cats) {
      var all = [];
  ...
  function initResources() {
    ...
    fetch("resources.json").then(function (r) { return r.json(); }).then(function (d) {
      ...
      resCount("count-templates", d.templates);
  ...
  function fetchCrew(start, end) {
    ...
    if (start && end) u += "?start=" + encodeURIComponent(start.toISOString()) + "&end=" + encodeURIComponent(end.toISOString());
    return fetch(u).then(function (r) { return r.ok ? r.json() : { configured: false, shifts: [], off: [] }; })
      .then(function (j) { return j || { configured: false, shifts: [], off: [] }; })
      .catch(function () { return { configured: false, shifts: [], off: [] }; });
  }
  ...
  function initSchedulePage(elView) {
    ...
    function render() {
      ...
      if (!lastCrew.configured) { elView.innerHTML = '<div class="note" style="margin:0">The field crew schedule isn\'t connected yet. Once QuickBooks Time is linked, shifts will appear here.</div>'; return; }
  ...
  function renderCrewPanel(el, crew, selected, emptyMsg) {
    if (!el) return;
    if (!crew.configured) { el.innerHTML = '<div class="note" style="margin:0">The field crew schedule isn\'t connected yet. Once QuickBooks Time is linked, shifts will appear here.</div>'; return; }
  ...
  function initManagerLinks() {
    ...
    var hrLink = document.getElementById("hr-letter-link");  // HR-only (employment letter)
    var salLink = document.getElementById("hr-salary-link"); // HR-only (salary/promotion letter)
    if (!link && !hrLink && !salLink) return;
    fetch("/.auth/me").then(function (r) { return r.ok ? r.json() : null; }).then(function (d) {
      var roles = (d && d.clientPrincipal && d.clientPrincipal.userRoles) || [];
      var hr = roles.indexOf("hr") > -1 || roles.indexOf("admin") > -1;
      if (link && (roles.indexOf("manager") > -1 || roles.indexOf("admin") > -1)) link.hidden = false;
      if (hrLink && hr) hrLink.hidden = false;
      if (salLink && hr) salLink.hidden = false;
    }).catch(function () {});
  }
  ...
  function initEmploymentLetter() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (!name) { statusEl.style.color = "#C0392B"; statusEl.textContent = "Employee legal name is required."; return; }
      var ohipEl = document.getElementById("el-ohip");
      var ohip = !!(ohipEl && ohipEl.checked);
      if (ohip && (!val("el-position").trim() || !val("el-start"))) {
        statusEl.style.color = "#C0392B"; statusEl.textContent = "OHIP letters must include the position and start date.";
        return;
      }
      var payload = {
      ...
      submitEl.disabled = true; statusEl.style.color = ""; statusEl.textContent = "Generating…";
      fetch("/api/employment-letter", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
        .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
        .then(function (res) {
          submitEl.disabled = false;
          if (res.ok && res.j && res.j.ok) {
            downloadBase64Pdf(res.j.pdfBase64, res.j.filename || "employment-letter.pdf");
            statusEl.style.color = "var(--gold-dark)";
            statusEl.textContent = "Letter downloaded." + (res.j.filed ? " Filed to the staff's HR folder (" + (res.j.filedTo || "matched folder") + ")." : " (Downloaded only — couldn't auto-match a staff folder to file it.)");
          } else {
            statusEl.style.color = "#C0392B";
            statusEl.textContent = (res.j && res.j.error === "HR only") ? "You need the HR role to generate letters." : "Couldn't generate the letter. Please try again.";
          }
        })
        .catch(function () { submitEl.disabled = false; statusEl.style.color = "#C0392B"; statusEl.textContent = "Couldn't generate the letter. Please try again."; });
    });
  }
  ...
  function initReviewForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      form.querySelectorAll(".rf-section").forEach(function (sec) {
        ...
        sec.querySelectorAll("[data-field]").forEach(function (f) {
          ...
          if (f.classList.contains("rf-score")) {
            ...
            if (isNumericScore(f)) { numRows.push(f); if (!val) missing++; }
          } else {
            val = (f.value || "").trim();
          }
        ...
        if (numRows.length) {
          ...
          rows.push({ label: "Average (÷ " + numRows.length + ")", value: (sum / numRows.length).toFixed(2) });
        }
      ...
      var payload = { form: form.getAttribute("data-form") || "", title: form.getAttribute("data-title") || "Review", version: form.getAttribute("data-version") || "", quarter: quarter, date: new Date().toLocaleDateString("en-CA"), sections: sections };
  ...
  function initRelianceLetter() {
    ...
    function showPreview(b64) {
      ...
      var wrap = document.getElementById("rl-preview-wrap"); if (wrap) { wrap.hidden = false; wrap.scrollIntoView({ behavior: "smooth", block: "start" }); }
    }
  ...
  function initSalaryLetter() {
    ...
    function updatePct() {
      ...
      if (prev && next) {
        ...
        pctEl.textContent = (p >= 0 ? "Increase" : "Change") + ": " + (r % 1 === 0 ? r : r.toFixed(1)) + "%   (" + money(prev) + " → " + money(next) + ")";
      } else { pctEl.style.display = "none"; }
    }
    ["sal-prev", "sal-new"].forEach(function (id) { var el = document.getElementById(id); if (el) el.addEventListener("input", updatePct); });
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (!name) { statusEl.style.color = "#C0392B"; statusEl.textContent = "Employee legal name is required."; return; }
      if (kind === "raise" && (!num("sal-prev") || !num("sal-new"))) { statusEl.style.color = "#C0392B"; statusEl.textContent = "Enter the previous and new amounts."; return; }
      if (kind === "promotion" && !val("sal-newpos").trim()) { statusEl.style.color = "#C0392B"; statusEl.textContent = "Enter the new position."; return; }
      var payload = {
        kind: kind, name: name, effectiveDate: val("sal-eff"),
        prevPosition: val("sal-prevpos").trim(), newPosition: val("sal-newpos").trim(),
        salaryBasis: val("sal-basis"), prevAmount: val("sal-prev").trim(), newAmount: val("sal-new").trim(),
      ...
      submitEl.disabled = true; statusEl.style.color = ""; statusEl.textContent = "Generating…";
      fetch("/api/salary-letter", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
        .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
        .then(function (res) {
          submitEl.disabled = false;
          if (res.ok && res.j && res.j.ok) {
            downloadBase64Pdf(res.j.pdfBase64, res.j.filename || "letter.pdf");
            statusEl.style.color = "var(--gold-dark)";
            statusEl.textContent = "Letter downloaded." + (res.j.filed ? " Filed to the staff's HR folder (" + (res.j.filedTo || "matched folder") + ")." : " (Downloaded only — couldn't auto-match a staff folder to file it.)");
          } else {
            statusEl.style.color = "#C0392B";
            statusEl.textContent = (res.j && res.j.error === "HR only") ? "You need the HR role to generate letters." : ((res.j && res.j.error) || "Couldn't generate the letter. Please try again.");
          }
        })
        .catch(function () { submitEl.disabled = false; statusEl.style.color = "#C0392B"; statusEl.textContent = "Couldn't generate the letter. Please try again."; });
    });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    initUser(); initWelcome(); initNav(); initTeamwork(); initNews(); initProcedures(); initResources(); initResTabs(); initMail(); initCalendar(); initProfile(); initReviews(); initManagerLinks(); initReviewForm(); initEmploymentLetter(); initSalaryLetter(); initReimbursement(); loadClaimHistory(); initPurchaseRequest(); loadPurchaseHistory(); initHrInquiry(); initHolidays(); initRelianceLetter();
  });
})();

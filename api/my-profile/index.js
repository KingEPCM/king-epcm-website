/*
 * GET /api/my-profile  → the signed-in person's Microsoft 365 directory details.
 * Read server-side (app-only Graph) so the profile page works on mobile.
 *
 * App settings: AAD_CLIENT_ID, AAD_CLIENT_SECRET, GRAPH_TENANT (optional).
 * Graph APPLICATION permission required (admin consent): User.Read.All
 */
...
const SELECT = "displayName,givenName,surname,jobTitle,department,officeLocation,mail,mobilePhone,businessPhones,employeeId";
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const me = await gget(token, "https://graph.microsoft.com/v1.0/users/" + encodeURIComponent(email) + "?$select=" + SELECT);
    context.res = json(200, me, { "Cache-Control": "no-store" });
  } catch (e) { context.log.error(e); context.res = json(502, { error: "Could not load profile" }); }
};
